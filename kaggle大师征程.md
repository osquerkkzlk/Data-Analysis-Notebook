...
data.reset_index()会把数据恢复到原来的索引，一般用于分组数据处理完后恢复到原形状,把 index（索引）转成正常的列，从而把一个“按某列分组后的 Series”变成“标准 DataFrame”。
...
在pandas中，category 是一种专门为**离散特征**设计的数据类型，它不是简单的字符串，而是用整数编码加映射表来存储的。把特征类型转换为  catrgory  后，xgboost等模型就可以自动识别并处理，不需要再手动做编码处理，减小预处理复杂度。同时，pandas内部进行了优化，使得该类型内存消耗小（只需要保存映射表和行索引标号），加速训练过程。category  的映射是无序的，虽然它会把离散特征映射为0、1、2...但是并不会引入顺序意义，pandas也会阻止索引进行大小比较。应用时，需要将其限定在非数值特征上，因为他也会强制转换数值类型，引发不可估计的后果。
...
该方法可以把数据重塑成一个二维表，其中，参数index，columns、values分别表示行列以及单元格的处理对象，参数 aggfunc表示对值对象的处理方法。
...
我们经常用到xgboost、lightgbm、catboost就属于此类。他们的核心思想都是去修正前面所有树的误差，但是他们都伴随着“过度专业化”的问题，也就是说在训练后期，新加入的树只对极少数样本起作用，而对大多数样本几乎不产生影响，即前几棵树学到了“全局特征”，后面的树越来越局部化，只在个别点上微调。这是因为前期的树容易拟合残差，随着残差逐渐减小，后期的树不容易拟合，因此为了减小残差，他们只能特化树的结构，学习极值点或异常点以去减小残差，这也就导致了后面的树越来越局部化。

这就导致了后期的树太专一，只在训练集上有效，并且因为后面的树贡献很小，整个模型几乎由最早的几棵树决定，容易陷入局部最优。之所有说后面的树贡献小，并不是说后期的树的权重小（学习率固定，所以权重相同），而是因为boosting的方法本身就决定了前面的树学习整体结构，奠定基础，后面的树微调，因此显得后期的树权重小。

因此，科学家提出了一种新的思想 “Dart“，类似于神经网络中的dropout技术，每次迭代中都随机丢弃一些树，使得最终的模型不会过度依赖前面的树，各树的贡献更加均衡（==**注意**==：在内部，LightGBM 在前 `1 / learning_rate` 次迭代中使用 `gbdt` 模式）。那是不是说我们一直使用Dart就好了？不尽然，因为dart最终预测需要使用全部的树加权求和，因此时间上比较慢，更适合过拟合风险高、结构复杂的情况，而GBDT更适合追求素的、稳定的情况，更适合作为基线来使用。总之，最后在stacking中都加入他们试试。

==对比  xgboost  和  lightgbm== ，xgboost是按深度来生长的，也就是他会对每一层的所有叶子都进行分裂，直到达到最大增益或者设定的最大深度或最小样本数，因此长出的树是对称的。而lightgbm 是按叶子来生长的，也就是他对所有叶子中分裂增益最大的叶子进行分裂，因此长出的树往往是不对称。

==lightgbm 超参数调优== 见[此处链接](https://lightgbm.readthedocs.io/en/stable/Parameters-Tuning.html) ，导入外部网页

我们来对比==catboost== 和上述的boosting算法 。首先，catboost采用了 “随机样本排序”，在训练开始时对样本进行了随机重排，减小了模型对样本顺序的依赖性。另一方面，catboost 采用了逐样本梯度的计算，也就是在构建第 i 棵树的时候，采用前 i-1 个样本去计算，这样避免了信息泄露与预测偏移（模型不会看到目标的信息，只会看到目标前面的样本的信息）。

与之相反，其他GBDT的实现，如 xgboost 和 lightgbm 等都会出现这种问题，尽管他们使用正则化降低偏移。这是因为，GBDT的训练是基于整个数据集来实现的，这也就导致计算第 i 个样本的残差时 ，模型会“间接”看到第 i 个样本的目标值，这是因为之前的模型是在整个数据集上训练得到的。但在预测阶段，模型无法访问新样本的目标值，所以性能会有所下降。简单来说，就是模型在训练阶段就已经见过了该样本，并且他对数据结构产生了一定影响，当对其进行训练时，模型的预测值在一定程度上反应了原始目标值，即造成了泄露。

==Catboost== 相关笔记见[此链接](#97、Catboost)，导入该笔记本目的位置



### 95、spearman 系数

在求解相关系数矩阵时，直接使用内核 ”spearman“，似乎这个内核比 "pearson"更优秀。
...
### 96、mean_squared_error

参数 squared默认是True，表示求mse，而如果设置为False则会求rmse。



### 97、Catboost

catboost的特点是接收缺失值，内部会对分类特征自动处理，但是不管数据中有没有分类特征都必须手动说明。
...
参数 train_dir 表示训练日志存储和学习曲线等文件的位置，通过  MetricVisualizer  可以可视化，即 MetricVisualizer  （'learing_rate_0.7'）.start()。

参数 eval_set 用于监控模型性能，一般情况下不会影响模型训练，但是如果开启早停或者设置了 use_best_model为True的话，会影响模型训练。你可以设置 early_stopping_rounds=20 来开启早停，表示如果20次迭代 validation_pool地分数都没有改善就停止。参数  use_best_model为True 在训练结束后会自动设置为最佳参数，而不是最后一次迭代的参数。他们通常配合使用。

参数    save_snapshot=True, snapshot_file='snapshot.bkp'，    snapshot_interval=1 用于配置快照功能，用于“断点再续”，如果笔记本突然崩溃，当你再次训练它的时候，会从该断点上继续训练。

参数   '*task_type='GPU'*' 可以设置为 gpu 上训练。

---

方法 model.tree_count_ 会返回训练的树的个数。catboost 有着和 sklearn 相同的接口，都有predict（）方法和predict_proba（）方法。

---

另一方面，你可以快捷地使用CV方法,其中 cv_data 存储着迭代次数和各项指标，是pd格式，可以直接显示。另一方面，你可以通过 sklearn 中的 cv 搜索方法去寻找最优参数。
...
metrics = model.eval_metrics(
    ...
    metrics=['Logloss','AUC'],
    ntree_start=0, # 0表示从第零棵树开始评估
    ntree_end=0, # 0表示直到最后一棵树
    eval_period=1, # 每过1轮就进行评估
    plot=True
)
# 你可以直接对metrics取对应地评估指标值，比如 你可以 metrics["AUC"]去得到每个iteration的AUC值

np.array(model.get_feature_importance(prettified=True))# 参数prettified表示是否显示列的名称
...
# 其中data制定了对什么数据进行评估，不同的评估数据结果也是不同的，推荐在验证集评估，参数type表示 评估模式，“LossFunctionChange”表示去掉某个特征后损失会怎么变化，侧面反映出特征重要性，这是非常严格的一个评估指标。其他指标如下。

```

| 类型                               | 含义                                     | 是否需要 `data` |
| ---------------------------------- | ---------------------------------------- | --------------- |
| `'PredictionValuesChange'`（默认） | 基于模型预测变化量计算的重要性（速度快） | ❌               |
| `'LossFunctionChange'`             | 基于损失函数变化量（更准确）             | ✅               |
| `'ShapValues'`                     | 基于 SHAP 理论（每样本解释性）           | ✅               |

---

保存模型和加载模型。由于 **文件扩展名不总能决定保存格式**（比如你可能写 `model.json` 但想保存二进制），所以 **必须明确告诉 CatBoost** 保存为 JSON 格式，此外也可以保存为onnx等格式的文件，如果选择保存为bin二进制文件，那么该文件只能被catboost读取。

可以通过load_model（）来加载**模型参数**。
...
反应各个特征对某个预测值的贡献度，模型会对贡献度进行公平叠加，最后得到预测值。
...
# recursive表示是否递归子目录
# pathname：一个字符串路径，可以包含通配符。

# 返回一个 文件路径列表
```

**常用通配符** ： * 会匹配任意长度的任意字符（不跨目录），？ 匹配任意单个字符，[......]会匹配字符集，**会递归匹配任意层级目录，但需要 设置参数  recursive=True。

```python
glob.iglob(pattern, recursive=False)

# 返回一个 迭代器（更节省内存,但在时间上和普通的 glob 差不多）。

for f in glob.iglob("**/*.txt", recursive=True):
    print(f)


# 适合文件非常多的情况，比如几千上万个文件。
```

```python
temp=glob.escape("name[1].txt")
# 自动在字符串中给特殊字符加反斜杠\,让他们失去通配符的特殊意义，让看起来像通配符的文件名便会普通字符串。
# 此时会返回 "name/[1/].txt"
...
```python
left.merge（right,on="id_col",how="outer"）
```

下面是 ”how“ 参数的用法：

- 'inner'：内连接，仅保留两边都匹配的行（默认）。
- 'left'：左连接，保留 left DataFrame 的所有行，right 中未匹配的填充 NaN。
- 'right'：右连接，保留 right DataFrame 的所有行，left 中未匹配的填充 NaN。
- 'outer'：全连接，保留两边的所有行，未匹配的填充 NaN。

​	

### 102、lightgbm 的早停机制

该机制只会监控 eval_set（训练过程中显示指标）中的最后一个指标。即使达到 early stopping 条件后模型继续训练了 N 轮没有提升，LightGBM 也会把最终模型回滚到最佳轮数对应的参数，所以 **不会因为后面的无效迭代改变模型**。



### 103、plt.legend()

该函数有参数 title=""可以设置图例名称，说明意义。



### 104、count（）和size（）对比

count会计数非空行，而size（）会计数所有行，包括NAN



### 105、pd和np传入函数

默认是浅拷贝的形式，也就是在函数中修改数据会直接影响原始数据。如果不想修改原始数据，传入函数时最好加上data.copy（）以进行深拷贝。



### 106、lgb

fit（）方法中有一些参数可以设置，eval_set参数可以设置评估数据，可以配合早停回调函数。callbacks可以设置早停回调函数、打印日志频率回调函数、实时重设参数回调函数。此外，还有参数 eval_metric表示打印日志的时候额外显示的指标，并不会改变模型的优化目标，categorical_features用于指定category的特征，但是如果你已经手动更改了数据的类型为category，那么lgb会自动检测。



### 107、optuna

Optuna 的 trial 是全局记录，而不区分模型，他会跟踪所有出现的trial.suggest。
...
作者把数据划分成了非数值列、布尔类型列、数值列来分别观察数据的分布情况。我的感受是直观、信息量大，美观，所以我觉得学习他的这种写法是很有价值的。
...
之后，作者对数值特征应用了相关系数，并绘制了热力图去初步窥探特征与目标之间的线性关系。再然后，作者分别检查了类别特征和数值特征的分布一致性。之后合并了数据，并进行了特征工程构造出4个特征，如下。

之后独具特色的是，它通过id来分离训练集和测试集，但是我觉得他在这一点上做的很乱，因为我想就没有必要先前把id和target分离，后面又加上，再分离训练，这里做的很乱。
...
总的来说似乎他的分数相对差一点，但是学习了他的数据分析过程，还是很有价值的，不是吗。

#### 4、大师三

第三位学习目标，当然就是整个日本小哥了，他在这次比赛中十分亮眼，写出了4、5本很不错的笔记本，那么接下来我将会一一学习他们，你知道的，毕竟时间不多了。

首先，在他的所有笔记本中，我发现作者在训练中很喜欢提前设定一些种子，然后在cv中取不同的种子训练模型并预测的结果平均值，也许这样子结果更稳定。

#### 5、大师四

在他的笔记本在构造特征方面有点东西。首先，在数值特征方面，他选取一些特征相乘或者相除用以表达更深层的含义，如限速和报道事故数相乘，曲率和车道数相除。在非数值特征方面，他构造两个特征相加，如下。此外，他还对相对重要的数值特征进行了分箱处理，对目标（连续值）进行了  log1p  处理。

    df["weather_time"]       = df["weather"] + "_" + df["time_of_day"]
    df["lighting_weather"]   = df["lighting"] + "_" + df["weather"]

另一方面，在进行 stacking 的时候，你需要确保基模型训练的cv相同，也就是设定相同的种子以及相同的其他参数，如果不这样的话会导致数据泄露，使得本地的分数较高，但是LB上的分数较差。具体原因是因为，当两个模型的cv划分不同时，A模型会通过其他折数据去预测某些数据，而B模型可能会根据不同的数据去预测某些数据，你要注意折数以及随机划分的特点，这就使得A模型预测 数据1 的时候用到了 数据2 （数据1 携带了 数据2 的信息），但是B模型预测数据1的时候并没有用到数据2（说明 数据1 和 数据2 是同一折）。这样的话，在进行 stacking 的时候，这些oof_preds（折外预测）作为普通特征输入，即使他们进行cv划分，也有极大可能会出现 数据1 去预测 数据2 的情况，但是数据1 已经携带了 数据2 的信息，就会使得结果过于乐观，造成本地分数高但是LB分数低的局面。
...
1、我注意到作者设置了 ORIG、BASE、TARGET来区别各个特征，便于处理以及让我们的思维更加直观，这是个不错的尝试。之后呢，他创建了交互特征，让我惊讶的是他也创造了非数值特征和数值特征的交互。
