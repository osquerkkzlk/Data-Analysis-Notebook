...
<img src="E:\typora_picture\image-20250627153151729.png" alt="image-20250627153151729" style="zoom: 80%;" />
...
<img src="E:\typora_picture\image-20250627153213340.png" alt="image-20250627153213340" style="zoom:80%;" />
...
<img src="E:\typora_picture\image-20250627153125555.png" alt="image-20250627153125555" style="zoom: 80%;" />
...
![image-20250627153045561](E:\typora_picture\image-20250627153045561.png)
...
![image-20250718113432603](E:\typora_picture\image-20250718113432603.png)
...
![image-20250705150508506](E:\typora_picture\image-20250705150508506.png)
...
**隐藏层**是网络的空间维度——处理“这一次输入”要经过哪些算子。 **隐状态**是网络的时间维度——上一次处理结果如何影响“下一次输入”。RNN 的参数矩阵（例如用于计算隐藏状态的权重）在每次训练结束后才会更新。由上一个隐状态和当前时间步输入得到。
...
![image-20250725170403684](E:\typora_picture\image-20250725170403684.png)

2. **多维梯度下降**

![   ](E:\typora_picture\image-20250725170636818.png)
...
| 符号      | 含义                                                       |
| --------- | ---------------------------------------------------------- |
| g ~t,t−1~ | 第 t 次迭代中，对上一轮权重 wt−1的梯度估计                 |
...
![image-20250725204620553](E:\typora_picture\image-20250725204620553.png)
...
![image-20250725205417840](E:\typora_picture\image-20250725205417840.png)
...
1. **单因子调度器**
   ...
   ![image-20250725211612794](E:\typora_picture\image-20250725211612794.png)
...
![image-20250725211655241](E:\typora_picture\image-20250725211655241.png)

2. **多因子调度器**

<img src="E:\typora_picture\image-20250725212913226.png" alt="image-20250725212913226" style="zoom:80%;" />
...
![image-20250725213350658](E:\typora_picture\image-20250725213350658.png)
...
==<  深度学习的互联方式  >==  ![image-20250727184633244](E:\typora_picture\image-20250727184633244.png)
...
首先将文本列表拆分成词元列表（单词词元或者字符词元）。词元的类型是字符串，将其映射到数字列表，叫做词表，对所有的唯一词元进行统计，得到的结果称为语料，语料库中不存在或已删除的任何词元都将映射到一个特定的未知词元“<unk>”
...
A @ B  # 元素点积
torch.mv(A,B)  # 矩阵-向量积,对AB的形状有严格要求，并且矩阵向量计算不会进行广播，一定程度上更安全
torch.mm(A,B)  # 矩阵乘法，且只能进行二维矩阵乘法，有严格限制，不会进行广播，在某些场景下会使用，也是为了确保安全。
```

![image-20250811212504070](E:\typora_picture\image-20250811212504070.png)
...
![image-20250811213436334](E:\typora_picture\image-20250811213436334.png)

 $\|\mathbf{x}\|_1 = \sum_{i=1}^n \left|x_i \right|.$ 为 L1 范数 ，$\|\mathbf{x}\|_2 = \sqrt{\sum_{i=1}^n x_i^2}$ 为L2范数，

![image-20250811213612962](E:\typora_picture\image-20250811213612962.png) 
...
![image-20250813104626628](E:\typora_picture\image-20250813104626628.png)
...
![image-20250815175816473](E:\typora_picture\image-20250815175816473.png)
...
![image-20250813184850091](E:\typora_picture\image-20250813184850091.png)
...
![image-20250819110830115](E:\typora_picture\image-20250819110830115.png)
...
仿射变换的特点是通过加权和对特征进行*线性变换*， 并通过偏置项来进行*平移*。
...
1. **损失函数求和，是为了把模型的输出变成一个标量**。
    深度学习的损失函数必须是标量，才能进行自动求导和参数更新。
    （求和是一种常见的方式，如 y.sum()）

2. **反向传播会自动计算标量损失对所有参数的梯度**。
    这些梯度表示：

3. > 每一个参数改变一点点后，损失会如何变化。
   >  梯度方向永远指向“损失增加最快”的方向。

4. **为了减小损失，我们沿着负梯度方向更新参数**。
    因为负梯度方向是“损失下降最快”的方向，这是方向导数定理保证的。所有参数的梯度都是“同时”参与更新的。

### 4、.numpy（）的局限

.numpy（）函数会把张量变为np数据，但是他并不会截断计算图，反而会直接产生报错。所以合适的方法是.detach().numpy()。
...
虽然softmax本身是非线性的，但是他对线性变换 WX+B 做变化，并不会改变分类决策的“线性结构”，换言之，他只是对输出进行了规范，将输出限定在（0，1）的范围上。

### 7、交叉熵

本质上是在衡量两个分布的差异  $H(y,p)=−i∑yilogpi$ ,当预测分布和真实分布一致时，损失会最小，这是由数学决定的。

### 8、正态分布

normal可以指定均值和标准差，而randn时标准正态分布。

### 9、np的广播

numpy具有广播的特点，pytorch的广播完全继承自numpy。

### 10、L1正则化与L2正则化对权重的影响

L1正则化 对梯度的导数是一个常数（0点除外），所以很容易把低权重推到0（大权重和小权重的梯度一样，优化时减小的幅度一样，所以更有可能趋于0）。L2正则化对梯度的导数不是常数，权重越大梯度越大（从而优化时走的步长越大），权重越小梯度越小（从而优化时走的步长越小），所以最后很容易呈现出权重均匀的结果。

并且，需要澄清的一点是，优化时各个参数都会沿着最大梯度的反方向前进。不同的参数的区别是前进的步长问题。

### 11、偏差-方差 平衡

1. **机器学习误差分解**
    对一个预测模型$ \hat{f}(x) $来预测真实值 $y$，总误差可以分解为：

$$
\mathbb{E}[(y - \hat{f}(x))^2]=Bias^2+Variance+Irreducible Error
$$

- **Bias（偏差）**
  - 衡量模型预测的期望与真实函数的差距
  - 偏差高 → 模型假设过于简单，无法拟合数据 → **欠拟合**
- **Variance（方差）**
  - 衡量模型在不同训练集上预测结果的变化程度
  - 方差高 → 模型对训练数据敏感 → **过拟合**
...
> 在模型复杂度和泛化能力之间找到一个平衡点，使总误差最小

直观理解：

1. **欠拟合**（偏差主导） → 模型太简单，训练误差大，测试误差也大
2. **过拟合**（方差主导） → 模型太复杂，训练误差低，但测试误差大
3. **最佳平衡点** → 模型既能拟合数据，又不对噪声过敏

### 12、Dropout

会以一定概率暂时失活神经元，虽然概率是固定的，但是神经元是随机选择的，相当于引入噪声，而我们知道引入噪声就相当于对数据加上L2正则化。

### 13、参数初始化

当我们对隐藏层初始化相同的参数时，他们的输出将会相同（激活相同），最终表现出来就像只有一个神经元一样，效果比较差。

### 14、卷积核权重

卷积核权重也是可学习参数，一开始是随机初始化的，在反向传播时更新参数。

### 15、平移不变性

本质：特征在图像任何位置都可以被检测到，但是他有很多缺点：卷积核不关心具体位置信息，只关心特征是否会出现，所以会丢失位置信息。卷积核识别到的是局部信息，很难理解全局结构。卷积核过度依赖局部特征，更关注纹理，不关心形状。这些缺点限制了他的能力。

虽然CNN通过层层堆叠增加了感受野，但是他并没有直接建立全局结构，而是将信息逐层传递，这就导致他最终只知道这个物体应该有A特征、B特征、C特征，但是并不知道他们的具体位置关系。

### 16、池化层

池化时，池化核进行遍历，如果窗口不足池化核的大小，那么该窗口就会被忽略。并且池化会减轻卷积对位置的敏感性，它不关注特征的精准位置，而只关心特征是否存在于某个大的区域中。

### 17、卷积层的格式

conv的参数设置顺序大致如右所示（in,out,kernel_size,padding、stride）。因为展平之后是线性层，这需要我们直到神经元数量，至此，我们有一些快捷的方法。一种是将一个随机元素输入进去，通过观察输出的形状确定，一种是采用self.avgpool = nn.AdaptiveAvgPool2d((1, 1))，即对每个通道做全局平均，从而最终的神经元数量就是输出通道数,另一种就是我们利用参数的关系手动计算形状。

### 18、BatchNormed（批量归一化层）

一般在卷积之后激活之前使用（放置在卷积之后，是因为卷积起到的作用是提取特征，直接规范化会破坏这种结构特征。但是如果不进行规范化，每一层的输入差异很大，梯度不稳定。放置在激活之前，是因为如果在激活后面不仅会破坏激活数值分布，而且规范化后数值很可能出现小于0的情况，甚至可能出现梯度消失的情况），通过计算当前batch的均值和方差（加上一些噪声扰动防止除0），来规范化当前batch的输入，同时会记录全局的均值和方差。

并且，BN本身有gamma和beta参数用于缩放和平移，这是因为直接使用标准化的数据直接限制了输入的范围，限制了模型的表达能力，而增加这两个可学习参数就会丰富表达能力。在评估阶段时，他会直接使用该均值和方差计算。

接下来是关于卷积层或线性层偏置的讨论。**如果层输出被多个地方直接使用**，而某些使用路径没有经过 BN（例如 skip-connection 在 BN 之前合并），那偏置会影响未通过 BN 的分支。**BN 未被放在该层之后**（比如你把 BN 放在激活之后，或根本不使用 BN），则不要删 bias。

### 19、序列模型

自回归模型是指我们使用固定长度的序列作为输入取预测当前值，然后继续这样做下去。隐变量自回归模型是指对过去进行总结并结合当前输入进行预测，因为过去的总结这一状态不被观测，是隐藏的，所以该方法被称为隐变量自回归模型。

### 20、循环神经网络

该网络的名称来源于隐状态，隐状态由前一个隐状态和当前输入计算求得，循环往复，因此得名。并且RNN采用的激活函数是tanh（范围在 -1 和 1 ），这是因为隐状态会被反而传递，如果激活值不进行限制，很容易梯度爆炸。

另一方面，torch选择元组作为隐状态类型，是因为元组不可修改、比较安全，并且还具有顺序性的特点。除此之外，为了防止梯度爆炸，RNN采用了每个batch梯度隔绝的方法，特点是原本长长的反向传播链被分解为若干个小的反向传播链，他们数值是接续的，但是梯度不接续。

然而，普通的RNN有一些缺陷，开始的词元可能很重要，但是由于长链反向传播导致梯度很小，使得该词元不那么重要；有时候一些词元无关紧要，但对预测产生较大影响；有时候，逻辑会中断，也就是前一句和后一句不存在逻辑关系，但是RNN不能改善。门控循环单元与普通的循环神经网络之间的关键区别在于： 前者支持隐状态的门控。

对于GRU来讲，重置门有助于捕获序列中的短期依赖关系，更新门有助于捕获序列中的长期依赖关系。重置门打开时，门控循环单元包含基本循环神经网络；更新门打开时，门控循环单元可以跳过子序列。

对于LSTM来讲，加入了输入们、遗忘门、输出们以及记忆单元，比较复杂，但效果和GRU差不多，所以选择GRU蛮好的。

对于双向循环神经网络来讲，前向传播需要用到过去和未来的数据，适合做完形填空，但是真正预测时，我们并没有未来的标签。

对于编码器和解码器来讲，编码器用于提取输入并压缩成状态，解码器把该状态解码成解码器可以接收的状态，然后同解码器输入一起送入解码器。其中编码器会返回输出以及最后一个时间步的隐状态。

我们在搜索过程中总是取最大的那个预测，这是典型的贪心算法，但是他不是最优的。因为有时候前面表现一般的词元，对后面的影响会使得整体的句子的概率更高。

### 21、CrossEntropyLoss

要求类别维在第二个维度，所以有时候需要premute一下。

### 22、注意力机制

注意力机制的核心思想是让模型在处理数据时专注于更重要的部分，而不是平等地对待所有的输入。常用的方法有加性注意力（查询和键的长度可以不等）和缩放点积注意力。Bahdanau注意力机制是在说：解码器预测每个词的时候并不需要看到很靠后的词元，只需要专注当前词元附近的词元就ok了，其中标签（目标词）作为quary，指导源数据的注意力权重，并将该时刻的隐状态和目标词结合起来一同送入解码器。

对于Transformer来讲，他的多头注意力机制键值和查询都是同一个输入，表示查看当前序列对每个位置的词的影响。首先，输入进入嵌入层和位置编码，因为嵌入层的输出很小而位置编码的结果在-1和1之间，直接相加，嵌入层的输出很可能被位置编码淹没，导致训练不稳定，所以需要让嵌入层输出乘上词向量幅度的根号值以匹配数值范围。

### 23、repeat 和 repeat_interleave对比

`repeat` 是「复制结构」，`repeat_interleave` 是「复制元素」。
...
| 参数名           | 意义                          | 应满足                            |
| ---------------- | ----------------------------- | --------------------------------- |
| `num_embeddings` | `vocab_size`（词表大小）      | 输入的 token_id ∈ [0, vocab_size) |
| `embedding_dim`  | `num_hiddens`（隐藏向量维度） | 自由选择，比如 32/64/256          |

| 步骤 | 目标                                   | 操作（直接复制粘贴）                                         | 预计耗时   |
| ---- | -------------------------------------- | ------------------------------------------------------------ | ---------- |
| 1    | 把模型训好并导出                       | 训练到 BLEU 26+（您现在已经很接近了） 训练完后加这几行保存： | 您已完成   |
|      |                                        | ```python                                                    |            |
| 2    | 转成 ONNX（加速 + 前端调用最简单）     | `bash<br>pip install onnx onnxruntime-gpu<br>python export_to_onnx.py   # 我下面给您完整脚本<br>` | 10 分钟    |
| 3    | 写一个超简洁的后端（FastAPI + ONNX）   | 就 50 行代码，我直接给您完整文件                             | 30 分钟    |
| 4    | 写一个更好看的网页前端（纯 HTML + JS） | 直接复制我写好的模板，零 JavaScript 经验也能用               | 30 分钟    |
| 5    | 本地测试能跑                           | `bash<br>uvicorn main:app --reload --port=8000<br>` 打开 [http://localhost:8000](http://localhost:8000/) | 10 分钟    |
| 6    | 一键免费部署到云端（推荐两个方案）     | 方案 A（完全免费 + 最快 5 分钟） → Railway / Fly.io / Render（新用户都送 5～10 美元） 方案 B（长期最便宜 + 最快） → 买个新加坡 5 SGD/月 VPS（Contabo / Hostinger） | 5～30 分钟 |
| 7    | 加个免费域名 + HTTPS（3 分钟）         | 用 [https://www.freenom.com](https://www.freenom.com/) 免费拿 .tk/.ml 域名 或者直接用 Cloudflare 免费 SSL | 3 分钟     |
| 8    | 发给朋友炫耀！                         | 发微信、Telegram、Twitter：“我自己训练的德英翻译上线啦！”    | 永久       |
